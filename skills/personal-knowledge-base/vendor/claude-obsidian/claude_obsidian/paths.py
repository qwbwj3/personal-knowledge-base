...
def _probe_wrapped_dirfd(function: object, operation: str) -> bool:
    """Verify a non-native wrapper's ``dir_fd`` behavior in an OS temp dir.

    ``os.supports_dir_fd`` is an identity set populated when Python starts.
    Runtime safety shims may later wrap ``os.unlink`` or ``os.rmdir`` while
    preserving their descriptor-relative behavior.  An identity-only check
    then reports a false negative.  We accept such a wrapper only after it
    actually removes a randomly named child relative to a pinned descriptor.
    The probe is confined to the operating-system temp directory and cached by
    wrapper identity; accepting a matching signature without exercising it
    would weaken the fail-closed confinement gate.
    """

    if operation not in {"unlink", "rmdir"} or not callable(function):
        return False
    name = f"probe-{os.getpid()}-{os.urandom(8).hex()}"
    try:
        with tempfile.TemporaryDirectory(prefix="claude-obsidian-dirfd-") as root:
            root_path = Path(root)
            target = root_path / name
            if operation == "unlink":
                target.write_bytes(b"dir-fd capability probe\n")
            else:
                target.mkdir()
            descriptor = os.open(
                root_path,
            ...
            try:
                function(name, dir_fd=descriptor)
            finally:
                os.close(descriptor)
...
def supports_dir_fd_call(function: object, *, operation: str | None = None) -> bool:
    """Return whether ``function`` demonstrably supports ``dir_fd``.

    Native functions retain CPython's authoritative membership result.  Only
    the two deletion functions known to be wrapped by host safety layers are
    eligible for the functional fallback, and only when the supplied callable
    is the process's current ``os`` function for that operation.
    """

    if function in os.supports_dir_fd:
        return True
    if operation == "unlink" and function in {os.unlink, os.remove}:
        return _probe_wrapped_dirfd(function, operation)
    if operation == "rmdir" and function is os.rmdir:
        return _probe_wrapped_dirfd(function, operation)
    return False
...
def supports_confined_dirfd() -> bool:
    ...
    return (
        ...
        and supports_dir_fd_call(os.open)
        and supports_dir_fd_call(os.mkdir)
        and supports_dir_fd_call(os.rename)
        and supports_dir_fd_call(os.stat)
        and supports_dir_fd_call(os.unlink, operation="unlink")
    )
